...
module.exports = {
  validateUserPost: [
    // Constraints must to be defined
    // Validate name
    body('firstName', 'Ingrese un nombre válido').exists().isLength({ min: 2 }),
    // Validate Suername
    ...
    body('password', 'La contraseña debe tener mas de 5 caracteres').isLength({
      min: 5,
    }),
    validateResult,
  ],
...
const validateUserRegister = [
  ...
  body('firstName', 'Ingrese un nombre válido').exists().isLength({ min: 2 }),
  // Validate Suername
  body('lastName', 'Ingrese un apellido válido').exists().isLength({ min: 2 }),
  // Validate Email
  body('email', 'Ingrese un email válido').exists().isEmail(),
  // Validate Password Length
  body('password', 'La contraseña debe tener mas de 5 caracteres').isLength({
